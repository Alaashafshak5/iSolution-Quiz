...
function add_custom_date_field_meta_box() {
    add_meta_box(
        'date_field',
        'Project Date',
        'display_date_field',
...
add_action('add_meta_boxes', 'add_custom_date_field_meta_box');
...
function remove_inline_style_from_post_thumbnail($html, $post_id, $post_thumbnail_id, $size, $attr) {
    $html = preg_replace('/style=("|\')(.*?)("|\')/','',$html);

    return $html;
}

add_filter('post_thumbnail_html', 'remove_inline_style_from_post_thumbnail', 10, 5);

function projects_shortcode_function() {
	$args = array(
		'post_type' => 'projects',
		'post_status' => 'publish',
		'posts_per_page' => -1, 
	...
	if ($query->have_posts()) {
		while ($query->have_posts()) {
			...
			$output .= '<a class="card" href="' . get_the_permalink() . '">';
			$output .= get_the_post_thumbnail();
			$output .= '<div class="date purple">' . $date . '</div>';
...
add_shortcode('projects', 'projects_shortcode_function');
